...
Hooks.once("init", function () {
  ...
  CONFIG.Combat.initiative = {
    formula:
      "{1d10x, 1d10x, 1d10x}dh1kh1 + @derivadas.iniciativa - @derivadas.estorbo",
    decimals: 0,
  };
...
Hooks.on("renderTokenHUD", async (hud, html, token) => {
  ...
  const buttonDisplay = await renderTemplate(
    "/systems/ryf/templates/tokens/extended-hud.html"
  );
  html
    .find("div.right")
    ...
    .click((event) => {
      ...
      for (const button of controlIcons) {
        ...
        if (button.dataset.action === "thwildcard-selector")
          tokenButton = button;
      }
...
Hooks.on("getSceneControlButtons", (controls) =>
  RyFExtras.leftSideBar(controls)
);
