...
export default class RyFActorSheet extends ActorSheet {
  ...
  _prepareCharacterItems(sheetData) {
    ...
    for (let i of sheetData.items) {
      ...
      if (i.type === "arma") {
        ...
        if (itemSystem.atributo == "F") {
          itemSystem.total =
            itemSystem.nivel + actorData.system.atributos.fisico;
        }
        if (itemSystem.atributo == "D") {
          itemSystem.total =
            itemSystem.nivel + actorData.system.atributos.destreza;
        }
        if (itemSystem.atributo == "I") {
          itemSystem.total =
            itemSystem.nivel + actorData.system.atributos.inteligencia;
        }
        if (itemSystem.atributo == "P") {
          itemSystem.total =
            itemSystem.nivel + actorData.system.atributos.percepcion;
        }
        if (itemSystem.atributo == "C") {
          itemSystem.total =
            itemSystem.nivel + actorData.system.atributos.carisma;
        }
        habilidades.push(i);
  ...
  _calculaValores(actorData) {
    ...
    puntosVida =
      Number(atributos.fisico) * Number(this.getHitPointsMultiplier(data));
    //CALCULO MANA
    puntosMana =
      Number(atributos.inteligencia) * Number(this.getManaMultiplier(data));
    //CALCULO INICIATIVA
    ...
    Defensa_Base =
      Number(atributos.destreza) + Number(val_esquivar) + Number(5);
    //CALCULO DEFENSA ESCUDO CaC y Dist
    let escudo = items.escudos.find(
      (k) => k.type === "escudo" && k.system.equipado
    );
    if (escudo) {
    ...
    let armadura = items.armaduras.find(
      (k) => k.type === "armadura" && k.system.equipado
    );
    if (armadura) {
    ...
    this.actor.update({
      "data.derivadas.defensa.escudoCac": Defensa_Escudo_CaC,
    });
    this.actor.update({
      "data.derivadas.defensa.escudoDist": Defensa_Escudo_Dist,
    });
  ...
  activateListeners(html) {
    ...
    html.find(".item-equip").click((ev) => {
      ...
      if (!objeto_a_equipar.system.equipado) {
        if (objeto_a_equipar.type === "arma") {
          equipados = this.actor.items.filter((item) => {
            return item.system.equipado && item.type === "arma";
          });
    ...
    html.find(".level-up").click((ev) => {
      ...
      let experienciaFinal =
        Number(this.actor.system.derivadas.experiencia.valor) -
        Number(siguienteNivel);
      if (experienciaFinal < 0) {
        ui.notifications.notify(
          "No tienes experiencia suficiente para subir de nivel esa habilidad."
        );
      } else {
        this.actor.update({
          "data.derivadas.experiencia.valor": experienciaFinal,
        });
    ...
    if (!this.actor.system.locked || game.user.isGM) {
      ...
      html.find(".item-create").click((ev) => {
        ui.notifications.warn(
          "No puedes agregar un item a la ficha mientras está bloqueada. Puedes arrastrar uno existente."
        );
      });
      ...
      html.find(".item-edit").click((ev) => {
        ui.notifications.warn(
          "No puedes editar un item de la ficha mientras está bloqueada. Puedes editar uno existente y luego arrastrarlo."
        );
      });
    }
  }
  ...
  async _onTiradaAtributo(event) {
    ...
    if (
      Number(this.actor.system.derivadas.puntosVida.value) <=
      Number(this.actor.system.atributos.fisico)
    ) {
      tipo_dado = "menor";
    }
    const archivo_template =
    ...
    const contenido_Dialogo = await renderTemplate(
      archivo_template,
      datos_template
    );
        ...
        Lanzar: {
          ...
          callback: () => {
            tiradaAtributo(
              ...
              tipo_dado,
              document.getElementById("forzar").value
            );
          },
        },
      },
  ...
  async _onTiradaHabilidad(event) {
    ...
    if (
      Number(this.actor.system.derivadas.puntosVida.value) <=
        Number(this.actor.system.atributos.fisico) ||
      Number(dataset.valor_habilidad) == 0
    ) {
      tipo_dado = "menor";
    }
    let val_atributo = 0;
    if (dataset.atributo == game.i18n.localize("RYF.attributes.strength")) {
      val_atributo = this.actor.system.atributos.fisico;
    }
    if (dataset.atributo == game.i18n.localize("RYF.attributes.dexterity")) {
      val_atributo = this.actor.system.atributos.destreza;
    }
    if (dataset.atributo == game.i18n.localize("RYF.attributes.intelligence")) {
      val_atributo = this.actor.system.atributos.inteligencia;
    }
    if (dataset.atributo == game.i18n.localize("RYF.attributes.perception")) {
      val_atributo = this.actor.system.atributos.percepcion;
    }
    if (dataset.atributo == game.i18n.localize("RYF.attributes.charisma")) {
      val_atributo = this.actor.system.atributos.carisma;
    }
    const archivo_template =
      "/systems/ryf/templates/dialogs/tiradaHabilidad.html";
    const datos_template = {
      nom_atributo: dataset.atributo,
  ...
  async _onTiradaHechizo(event) {
    ...
    if (
      Number(this.actor.system.derivadas.puntosVida.value) <=
        Number(this.actor.system.atributos.fisico) ||
      Number(dataset.nivel) == 0
    ) {
      tipo_dado = "menor";
    }
    let val_atributo = this.actor.system.atributos.inteligencia;
    const archivo_template =
      "/systems/ryf/templates/dialogs/tiradaHechizo.html";
    const datos_template = {
      val_atributo: val_atributo,
        ...
        Lanzar: {
          ...
          callback: () => {
            tiradaHechizo(
              ...
              document.getElementById("bonos").value,
              dataset.dificultad,
              document.getElementById("tipo_dado").value,
              dataset.mana
            );
          },
        },
  ...
  async _onTiradaArma(event) {
    ...
    if (objetivo) {
      ...
      if (objetivoActor.system.derivadas.defensa.escudoDist) {
        defensaDistanciaEscudo =
          objetivoActor.system.derivadas.defensa.escudoDist;
      }
    ...
    if (dataset.tipo == "melee") {
      ...
      HabilidadArma = this.actor.items.find(
        (k) => k.type === "habilidad" && k.name === "Armas Cuerpo a Cuerpo"
      );
      if (HabilidadArma) {
    ...
    if (dataset.tipo == "ranged") {
      ...
      archivo_template = "/systems/ryf/templates/dialogs/tiradaArma.html";
      HabilidadArma = this.actor.items.find(
        (k) => k.type === "habilidad" && k.name === "Armas a Distancia"
      );
      if (HabilidadArma) {
        valHabilidad = HabilidadArma.system.nivel;
      }
        ...
        Ataque: {
          ...
          callback: () => {
            ...
            tiradaAtaque(
              ...
              objetivo_id,
              absorcionArmadura,
              dataset.danobonificador,
              dataset.danonumerodados
            );
          },
        },
        Dano: {
  ...
  async _onRestauraVida(event) {
    ui.notifications.notify(
      game.user.name + " restaura la vida de " + this.actor.name
    );
    this.actor.update({
      "system.derivadas.puntosVida.value":
        this.actor.system.derivadas.puntosVida.max,
    });
  }

  async _onRestauraMana(event) {
    ui.notifications.notify(
      game.user.name + " restaura el mana de " + this.actor.name
    );
    this.actor.update({
      "system.derivadas.puntosMana.value":
        this.actor.system.derivadas.puntosMana.max,
    });
  }
  ...
  async _onSwitchLock(event) {
    ...
    ui.notifications.notify(
      game.user.name +
        (this.actor.system.locked ? " desbloquea" : " bloquea") +
        " la ficha de " +
        this.actor.name
    );
  ...
  loadSettings() {
    ...
    settings.hitPointsMultiplier = game.settings.get(
      "ryf",
      "hitPointsMultiplier"
    );
  ...
  getHitPointsMultiplier(actorData) {
    ...
    //TODO flags
    //actorData.items.filter(item => item.efectos["hitPointsMultiplier"]);
    return multiplier;
  }
